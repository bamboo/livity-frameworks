...
namespace Livity.Collections.Tests
{
	...
	public class PieceTableBenchmarks
	{
		...
		public void IntensiveEditingSession()
		{
			...
			{
				var insertionPoint = random.Next(table.Count);
				for (var i = 0; i < insertionsPerIteration; ++i)
					table = table.Insert(insertionPoint + i, i);

				var readPosition = random.Next(table.Count);
				table.ToArray(readPosition, Math.Min(table.Count - readPosition, insertionsPerIteration));

				for (var i = 0; i < deletionsPerIteration; ++i)
					table = table.Delete(random.Next(table.Count), 1);
			});
			...
			Assert.AreEqual(iterations*insertionsPerIteration - iterations*deletionsPerIteration, table.Count);
		}
		...
		public void SequentialAccessAfterManyDeletions()
		{
			...
			for (var i = 0; i < numberOfDeletions; ++i)
				table = table.Delete(randomizer.Next(table.Count), 1);

			Console.WriteLine("** CopyTo iteration **");
			var buffer = new char[1];
			var totalWithout = BenchmarkIterations(iterations, delegate
			{
				var position = randomizer.Next(table.Count - numberOfCharacters * 2) + numberOfCharacters;
